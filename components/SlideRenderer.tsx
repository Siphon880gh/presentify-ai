...
const SlideRenderer: React.FC<SlideRendererProps> = ({ slide, onUpdate, isActive, onRegenerateImage, isImageLoading, transitionType = SlideTransition.FADE, disableTransitions = false, isEditMode = false }) => {
  ...
  const ImageComponent = () => (
    <div className="bg-gray-50 rounded-xl overflow-hidden flex flex-col relative shadow-inner group h-full z-10">
      <div 
        className={`relative flex items-center justify-center ${slide.imageWidth || slide.imageHeight ? 'flex-none' : 'flex-1 min-h-0'}`}
        style={{
